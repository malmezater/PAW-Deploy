...
# --- 3. Inaktivera OneDrive autostart ---
Write-Host "Disabling OneDrive autostart..." -ForegroundColor Yellow

# Inaktivera via registret (gäller alla användare vid inloggning)
$oneDriveRunKey = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
if (Get-ItemProperty -Path $oneDriveRunKey -Name "OneDrive" -ErrorAction SilentlyContinue) {
    Remove-ItemProperty -Path $oneDriveRunKey -Name "OneDrive" -ErrorAction SilentlyContinue
    Write-Host "Removed OneDrive from HKCU Run key"
}

# Inaktivera via Task Scheduler
$task = Get-ScheduledTask -TaskName "OneDrive Standalone Update Task*" -ErrorAction SilentlyContinue
if ($task) {
    Disable-ScheduledTask -TaskName $task.TaskName -ErrorAction SilentlyContinue
    Write-Host "Disabled OneDrive scheduled task: $($task.TaskName)"
}
