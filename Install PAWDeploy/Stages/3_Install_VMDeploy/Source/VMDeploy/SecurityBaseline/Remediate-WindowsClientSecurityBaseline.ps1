...
function Set-RegistryDwordValue {
    ...
    try {
        ...
        # PAWDeploy fix (2026-09-22): the original "(Get-ItemProperty ... -ErrorAction
        # SilentlyContinue).$Name" throws "The property '...' cannot be found on this object" under
        # Set-StrictMode -Version Latest (see top of file) whenever $Name doesn't exist yet under
        # $Path - which is the common case on a fresh, never-managed machine (exactly what this
        # toolkit targets). Get-ItemPropertyValue + try/catch doesn't have that problem.
        $current = $null
...
function Set-RegistryStringValue {
    ...
    try {
        ...
        # PAWDeploy fix (2026-09-22) - see Set-RegistryDwordValue above for why.
        $current = $null
        try { $current = Get-ItemPropertyValue -Path $Path -Name $Name -ErrorAction Stop } catch { }
